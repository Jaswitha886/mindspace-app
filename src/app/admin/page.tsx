...
export default async function AdminDashboard({
  ...
  const busiest = Math.max(
    1,
    ...load.filter((c) => c.reportable).map((c) => c.sessions),
  );
  const anyLoadReportable = load.some((c) => c.reportable);
  const anyDeptReportable = depts.some((d) => d.reportable);
  const scopedReportable = departmentId
    ? depts[0]?.reportable ?? false
    : true;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <AdminDashboardClient
        ...
        totalStudentsAllTime={totalStudentsAllTime}
      ...
      <SuspensionStudents
        ...
        suspensions={suspensions.map((suspension) => ({
          ...
          student: {
            ...
            registerNumber:
              suspension.student.studentProfile?.registerNumber ?? null,
          },
